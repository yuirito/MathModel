...
\keywords{机器学习，多元回归，LSTM}%{针对具体的问题列一到两个关键字\quad  建模算法列出\quad }
\end{abstract}
...
在污染物排放情况不变的条件下，某一地区的气象条件有利于污染物扩散或沉降时，该地区的AQI会下降，反之会上升。由此分析出，在不同气象条件下的AQI指数可以很好地反应气象条件对污染物浓度的影响。与此同时，也要分别分析6种污染物(CO,NO2,SO2,O3,PM10,PM2.5)在不同气象条件下的浓度变化情况。因此，如何自定义一个变量用于标志各种不同的污染物对环境恶劣程度的影响。分析思路如下：首先计算AQI变量与各个污染物变量的相关性,然后通过主客观集成赋权法得到6种污染物浓度分别对我们最终定义的环境恶劣程度变量(称为AP)的影响权重。再通过各个污染物成为首要污染物的比例，对影响权重进行修正。最后通过相关性分析与插值法建模得到气象因素对AP的影响，我们可以将气象条件因素分为三类：有较为显著的单向影响因素，有较为显著的双向影响因素，无显著影响因素。对三种类别的解释如下：
\begin{itemize}
...
（1）有较为显著的单向影响类别：在定义域全区间内，随着变量值的上升，变量AP的取值不断上升或下降。也即在全定义域区间内都为正相关或负相关。
...
\begin{table}[h!]
	...
	\resizebox{.95\columnwidth}{!}{
		\begin{tabular}{|c|c|c|c|c|c|c|c|c|c|}
			...
			\hline
			2020/7/14&监测点A&7.90&41.13&53.02&22.39&46.77&0.61&52&NOP2\&PM10\\
			\hline
			2020/7/15&监测点A&9.80&42.16&54.02&48.52&48.61&1.10&67&PM2.5\\
			\hline
			\hline
			2020/7/13&监测点B&6.06&43.60&41.28&35.92&86.76&0.73&55&NO2\\
			\hline
			2020/7/14&监测点B&7.90&41.13&53.02&22.39&46.77&0.61&52&NOP2\&PM10\\
			\hline
			2020/7/15&监测点B&9.80&42.16&54.02&48.52&48.61&1.10&67&PM2.5\\
			\hline
			2020/7/13&监测点C&6.06&43.60&41.28&35.92&86.76&0.73&55&NO2\\
			\hline
			2020/7/14&监测点C&7.90&41.13&53.02&22.39&46.77&0.61&52&NOP2\&PM10\\
			\hline
			2020/7/15&监测点C&9.80&42.16&54.02&48.52&48.61&1.10&67&PM2.5\\
			\hline
		\end{tabular}
	}
	\end{center}
...
\subsubsection{问题描述和分析}
因为相邻区域之间的污染物通常具有相关性，因此通过建立一个多区域的协同模型能够提高预测精度。我们利用附件1和3中，监测点A，A1，A2和A3的一次模型预测数据和实测数据再加上问题三中已经训练好了模型，我们只用在原来的训练集上再加上监测点A1，A2和A3与监测点A组成一层，然后再加上一个卷积层来把4个监测站的数据关联起来进行协同训练。
\subsubsection{模型建立与求解}
